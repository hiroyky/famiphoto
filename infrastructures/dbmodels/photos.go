...
type photoR struct {
	...
	Exifs           ExifSlice           `boil:"Exifs" json:"Exifs" toml:"Exifs" yaml:"Exifs"`
...
func (r *photoR) GetExifs() ExifSlice {
	if r == nil {
		return nil
	}
	return r.Exifs
...
func (r *photoR) GetPhotoFiles() PhotoFileSlice {
	if r == nil {
		return nil
	}
	return r.PhotoFiles
...
func (r *photoR) GetPhotoThumbnails() PhotoThumbnailSlice {
	if r == nil {
		return nil
	}
...
var (
	photoAllColumns            = []string{"photo_id", "name", "imported_at", "created_at", "updated_at", "group_id", "owner_id", "file_name_hash"}
	photoColumnsWithoutDefault = []string{"name", "imported_at", "created_at", "updated_at", "group_id", "owner_id", "file_name_hash"}
...
func (o *Photo) Exifs(mods ...qm.QueryMod) exifQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`exif`.`photo_id`=?", o.PhotoID),
	)

	return Exifs(queryMods...)
}
...
func (o *Photo) PhotoFiles(mods ...qm.QueryMod) photoFileQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`photo_files`.`photo_id`=?", o.PhotoID),
	)
...
func (o *Photo) PhotoThumbnails(mods ...qm.QueryMod) photoThumbnailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`photo_thumbnails`.`photo_id`=?", o.PhotoID),
	)
...
// LoadExifs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (photoL) LoadExifs(ctx context.Context, e boil.ContextExecutor, singular bool, maybePhoto interface{}, mods queries.Applicator) error {
	var slice []*Photo
	var object *Photo
	...
	query := NewQuery(
		qm.From(`exif`),
		qm.WhereIn(`exif.photo_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load exif")
	}

	var resultSlice []*Exif
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice exif")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on exif")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for exif")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.PhotoID == foreign.PhotoID {
				local.R.Exifs = append(local.R.Exifs, foreign)
				if foreign.R == nil {
					foreign.R = &exifR{}
				}
...
// LoadPhotoFiles allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (photoL) LoadPhotoFiles(ctx context.Context, e boil.ContextExecutor, singular bool, maybePhoto interface{}, mods queries.Applicator) error {
	var slice []*Photo
	var object *Photo
	...
	query := NewQuery(
		qm.From(`photo_files`),
		qm.WhereIn(`photo_files.photo_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.PhotoFiles = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.PhotoID == foreign.PhotoID {
				local.R.PhotoFiles = append(local.R.PhotoFiles, foreign)
				if foreign.R == nil {
					foreign.R = &photoFileR{}
				}
				foreign.R.Photo = local
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (photoL) LoadPhotoThumbnails(ctx context.Context, e boil.ContextExecutor, singular bool, maybePhoto interface{}, mods queries.Applicator) error {
	var slice []*Photo
	var object *Photo
	...
	query := NewQuery(
		...
		qm.WhereIn(`photo_thumbnails.photo_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddExifs adds the given related objects to the existing relationships
// of the photo, optionally inserting them as new records.
// Appends related to o.R.Exifs.
...
func (o *Photo) AddExifs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Exif) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.PhotoID, rel.ExifID}
	...
	if o.R == nil {
		...
		o.R.Exifs = append(o.R.Exifs, related...)
	}
...
// AddPhotoFiles adds the given related objects to the existing relationships
// of the photo, optionally inserting them as new records.
// Appends related to o.R.PhotoFiles.
// Sets related.R.Photo appropriately.
func (o *Photo) AddPhotoFiles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PhotoFile) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.PhotoID, rel.PhotoFileID}
	...
	if o.R == nil {
		...
		o.R.PhotoFiles = append(o.R.PhotoFiles, related...)
	}
...
func (o *Photo) AddPhotoThumbnails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PhotoThumbnail) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.PhotoID, rel.PhotoID, rel.ThumbnailName}
...
func (o *Photo) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return PhotoExists(ctx, exec, o.PhotoID)
}
