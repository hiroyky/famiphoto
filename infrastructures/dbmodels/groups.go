...
func (r *groupR) GetPhotoFiles() PhotoFileSlice {
	if r == nil {
		return nil
	}
	return r.PhotoFiles
...
func (o *Group) PhotoFiles(mods ...qm.QueryMod) photoFileQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`photo_files`.`group_id`=?", o.GroupID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (groupL) LoadPhotoFiles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGroup interface{}, mods queries.Applicator) error {
	var slice []*Group
	var object *Group
	...
	query := NewQuery(
		qm.From(`photo_files`),
		qm.WhereIn(`photo_files.group_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Group appropriately.
func (o *Group) AddPhotoFiles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PhotoFile) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.GroupID = o.GroupID
			...
			values := []interface{}{o.GroupID, rel.PhotoFileID}
...
func (o *Group) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return GroupExists(ctx, exec, o.GroupID)
}
