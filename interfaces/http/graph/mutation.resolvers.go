...
func (r *mutationResolver) UploadPhoto(ctx context.Context, input *model.UploadPhotoInput) (*model.PhotoUploadInfo, error) {
	sess, ok := ctx.Value(config.ClientSessionKey).(*entities.OauthSession)
	if !ok {
		return nil, fperrors.New(fperrors.UserUnauthorizedError, nil)
	}
	...
	sign, err := r.photoImportUseCase.GenerateUploadURL(ctx, sess.UserID, groupID, time.Now())
	if err != nil {
		return nil, err
	}
