...
func (userPasswordL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserPassword interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeUserPassword.(*UserPassword)
		if !ok {
			object = new(UserPassword)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeUserPassword)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeUserPassword))
			}
		}
	} else {
		s, ok := maybeUserPassword.(*[]*UserPassword)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeUserPassword)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeUserPassword))
			}
		}
	}
...
func (o *UserPassword) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(userPasswordAllColumns, strmangle.SetIntersect(insert, update))
...
func (o *UserPassword) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return UserPasswordExists(ctx, exec, o.UserID)
}
