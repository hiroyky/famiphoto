...
func (r *oauthClientRedirectURLR) GetOauthClient() *OauthClient {
	if r == nil {
		return nil
	}
...
var (
	...
	oauthClientRedirectURLColumnsWithoutDefault = []string{"oauth_client_id", "redirect_url"}
	oauthClientRedirectURLColumnsWithDefault    = []string{"oauth_client_redirect_url_id", "created_at", "updated_at"}
...
func (oauthClientRedirectURLL) LoadOauthClient(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOauthClientRedirectURL interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeOauthClientRedirectURL.(*OauthClientRedirectURL)
		if !ok {
			object = new(OauthClientRedirectURL)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeOauthClientRedirectURL)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeOauthClientRedirectURL))
			}
		}
	} else {
		s, ok := maybeOauthClientRedirectURL.(*[]*OauthClientRedirectURL)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeOauthClientRedirectURL)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeOauthClientRedirectURL))
			}
		}
	}
...
func OauthClientRedirectUrls(mods ...qm.QueryMod) oauthClientRedirectURLQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`oauth_client_redirect_urls`.*"})
	}

	return oauthClientRedirectURLQuery{q}
}
...
func (o *OauthClientRedirectURL) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.OauthClientRedirectURLID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == oauthClientRedirectURLMapping["oauth_client_redirect_url_id"] {
		goto CacheNoHooks
	}
...
func (o *OauthClientRedirectURL) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(oauthClientRedirectURLAllColumns, strmangle.SetIntersect(insert, update))
...
func (o *OauthClientRedirectURL) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return OauthClientRedirectURLExists(ctx, exec, o.OauthClientRedirectURLID)
}
