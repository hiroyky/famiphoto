...
CREATE TABLE IF NOT EXISTS `famiphoto_db`.`photo_files` (
  ...
  INDEX `fk_photo_files_gropus_idx` (`group_id` ASC) VISIBLE,
  INDEX `fk_photo_files_users_idx` (`owner_id` ASC) VISIBLE,
  CONSTRAINT `fk_photo_files_photos`
    FOREIGN KEY (`photo_id`)
    REFERENCES `famiphoto_db`.`photos` (`photo_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_photo_files_gropus`
    FOREIGN KEY (`group_id`)
    REFERENCES `famiphoto_db`.`groups` (`group_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_photo_files_users`
    FOREIGN KEY (`owner_id`)
    REFERENCES `famiphoto_db`.`users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `famiphoto_db`.`photo_thumbnails` (
  `photo_id` INT NOT NULL,
  `thumbnail_name` VARCHAR(900) NOT NULL,
  `file_path` TEXT NOT NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `group_id` VARCHAR(128) NOT NULL,
  ...
  INDEX `fk_photo_thumbnails_groups_idx` (`group_id` ASC) VISIBLE,
  INDEX `fk_photo_thumbnails_users_idx` (`owner_id` ASC) VISIBLE,
  CONSTRAINT `fk_photo_thumbanils_photos`
    FOREIGN KEY (`photo_id`)
    REFERENCES `famiphoto_db`.`photos` (`photo_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_photo_thumbnails_groups`
    FOREIGN KEY (`group_id`)
    REFERENCES `famiphoto_db`.`groups` (`group_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_photo_thumbnails_users`
    FOREIGN KEY (`owner_id`)
    REFERENCES `famiphoto_db`.`users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
